  ...
  lifeExpectancy() {
    ...
    if (this.userAge <= this.expectancy) {
      ...
      for (var a = 0; a < yearsArray.length; a++) {
        expectancyArray.push(Math.floor(yearsArray[a] * this.expectancy));
      }
      for (var b = 0; b < expectancyArray.length; b++) {
        totalArray.push(Math.floor(expectancyArray[b] - userAgeArray[b]));
      }
    } else {
      for (var c = 0; c < yearsArray.length; c++) {
        userAgeArray.push(Math.floor(yearsArray[c] * this.userAge));
      }
      for (var d = 0; d < yearsArray.length; d++) {
        expectancyArray.push(Math.floor(yearsArray[d] * this.expectancy));
      }
      for (var e = 0; e < userAgeArray.length; e++) {
        totalArray.push(Math.floor(userAgeArray[e] - expectancyArray[e]));
      }
    }
    return totalArray;
  }
